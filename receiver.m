...
function [audio_recorder] = receiver(fc)
    ...
    time_value = 0.01; % How often the function should be called in seconds
...
function audioTimerFcn(recObj, event, handles)
    ...
    fs = 16000;                                                 % Sampling frequency
    const = [1+1j, -1+1j, 1-1j, -1-1j] / sqrt(2);    % QPSK (Gray coded)
    ...
    span = 8;                                                   % Set span = 6
    [pulse, t] = rtrcpuls(alpha,Ts,fs,span);                    % Create root raised-cosine pulse
